define(['module', 'jquery', 'ol', 'URIjs/URI'], function (module, $, ol, URI) {
    ...
    PublicaMundi.Maps.registerResource = function (type, title, reader, builder) {
        if (!type) {
        ...
        if (typeof builder !== 'function') {
            throw 'Parameter builder is not a function.';
        }
    ...
    PublicaMundi.Maps.Resources.Resource = PublicaMundi.Class({
        initialize: function (options) {
            PublicaMundi.extend(this.values, options);
        },
    ...
    PublicaMundi.Maps.Resources.LayerBuilder = PublicaMundi.Class({
        initialize: function (options) {
            PublicaMundi.extend(this.values, options);
        },
        ...
        renderLayerItem: function (resource, layer) {
            ...
            return '<img src="' + legend + '" alt="" class="legend" /><p><a class="ui-btn ui-btn-icon-notext ui-icon-edit btn-layer-config" data-layer="' + layer.id + '" href="#" style="float: left;"></a><span class="layer-title">' + layer.title + '</span></p><label class="layer-selector ui-li-aside"><input type="checkbox" data-layer="' + layer.id + '"></label>';
        },
    ...
    PublicaMundi.Maps.Resources.ResourceManager = PublicaMundi.Class(PublicaMundi.Maps.Observable, {
        ...
        addResource: function (options) {
            ...
            if (typeof serviceLocator[options.type] !== 'object') {
                throw 'Resource type is not supported.';
            }
            options.config = this.values.config;

            var reader = new serviceLocator[options.type].reader(options);

            var self = this;

            reader.getMetadata(function (metadata) {
        ...
        getResourceByTypeAndKey: function (type, key) {
            if (typeof serviceLocator[type] === 'undefined') {
                throw 'Resource type is not supported.';
            }
            if (!key) {
                return null;
        ...
        getResourceTypes: function () {
            ...
            for (var type in serviceLocator) {
                types.push({
                    type: type,
                    title: serviceLocator[type].title
                });
            }
        ...
        createLayer: function (map, id) {
            ...
            if ((resource) && (layer)) {
                var builder = new serviceLocator[resource.metadata.type].builder({
                    config: this.values.config
                });

                layer.viewer.visible = true;

                builder.create(map, resource, layer);
            }
        },
        destroyLayer: function (map, id) {
            ...
            if ((resource) && (layer)) {
                ...
                builder.destroy(map, resource, layer);
            }
        },
        getBuilder: function (type) {
            return new serviceLocator[type].builder({
                config: this.values.config
            });
        }
    });
    ...
    PublicaMundi.Maps.registerViewBuilder = function (type, constructor) {
        ...
        viewBuilders[type] = constructor;
    };

    PublicaMundi.Maps.Resources.UI.ViewBuilder = PublicaMundi.Class({
        initialize: function (options) {
            PublicaMundi.extend(this.values, options);
        },
        build: function () {
    ...
    PublicaMundi.Maps.Resources.UI.ViewManager = PublicaMundi.Class({
        ...
        createView: function (options) {
            if (typeof viewBuilders[options.resourceType] !== 'function') {
                throw 'View type is not supported.';
            }

            var buidler = new viewBuilders[options.resourceType]({
                config: this.values.config
            });
    ...
    PublicaMundi.Maps.UI.ResourceConfigView = PublicaMundi.Class(PublicaMundi.Maps.UI.View, {
        initialize: function (options) {
